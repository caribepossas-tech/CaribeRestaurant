                ...
                <div>
                    ...
                    <div class="h-auto pb-4 pt-3 select-none w-full">
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            ...
                            @if (($order->status == 'billed' || $order->status == 'payment_due') && user_can('Delete Order'))
                                <button
                                        ...
                                        <td
                                            ...
                                            @else
                                                <div class="flex flex-col items-end gap-1">
                                                    <span>{{ $item->created_at->timezone(timezone())->translatedFormat('d M, Y h:i A') }}</span>
                                                    @if($item->payment_method == 'offline' && $item->receipt)
                                                        <a href="{{ asset_url_local_s3($item->receipt) }}" target="_blank" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-xs flex items-center gap-1">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                            </svg>
                                                            @lang('modules.order.viewReceipt')
                                                        </a>
                                                        @if($order->status == 'payment_due')
                                                            <x-button wire:click="approvePayment({{ $order->id }})" class="mt-1 bg-green-600 hover:bg-green-700 text-white text-xs px-2 py-1">
                                                                @lang('modules.order.approvePayment')
                                                            </x-button>
                                                        @endif
                                                    @endif
                                                </div>
