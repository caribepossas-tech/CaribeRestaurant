<div
    ...
    <div class="grid ">
        @if ($mailDriver == 'smtp')
            @if(!app()->environment(['kosari','demo']))
                <x-alert type="danger" icon="info-circle">
                    <p class="text-sm">
                        Parece que ha cambiado el <code class="font-bold bg-gray-200 text-gray-800 px-1 py-0.5 rounded">APP_ENV=kosari</code> a otra cosa en
                        <code class="font-bold bg-gray-200 text-gray-800 px-1 py-0.5 rounded">.env</code>
                        archivo. Por favor, no lo cambie, de lo contrario, los detalles SMTP a continuación no se tomarán de aquí.
                    </p>
                </x-alert>
            @endif
            @if ($verified)
